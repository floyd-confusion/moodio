...
import logging
import datetime
from filters import FILTER_REGISTRY, FILTER_CONFIG
...
class Dataset:
    ...
    def __init__(self):
        ...
        self.radius_multiplier_factor = 2.0  # How much to scale radius when reduction > 50%
    ...
    def get_random_track(self):
        ...
        logger.debug(f"Selected track: {track['track_name']} by {track['artists']} (shown: {len(self.shown_tracks)} total)")
        return {
            'track_id': track_id,
    ...
    def adjust_pool(self, adjustment):
        ...
        # Check for contradicting filters and drop both if found
        contradicting_id = self._get_contradicting_adjustment(adjustment)
        has_contradiction = False
        
        if contradicting_id is not None:
            # Check if contradicting filter exists in queue
            for filter_record in self.filter_queue:
                if filter_record['adjustment_id'] == contradicting_id:
                    has_contradiction = True
                    break
        
        if has_contradiction:
            # Drop both the existing contradicting filter and don't add the new one
            original_length = len(self.filter_queue)
            self.filter_queue = [
                filter_record for filter_record in self.filter_queue
                if filter_record['adjustment_id'] != contradicting_id
            ]
            removed_count = original_length - len(self.filter_queue)
            logger.info(f"Contradiction detected: dropped {removed_count} existing filter(s) (adjustment_{contradicting_id}) and not adding new filter (adjustment_{adjustment})")
            removed_filters = [f"adjustment_{contradicting_id}", f"adjustment_{adjustment}"]
        else:
            # No contradiction, add filter to queue normally
            filter_record = {
                'timestamp': datetime.datetime.now().isoformat(),
                'adjustment_id': adjustment,
                'filter_name': filter_name
            }
            self.filter_queue.append(filter_record)
            removed_filters = []
        ...
        # Add information about removed contradicting filters
        if removed_filters:
            response_data['contradicting_filters_removed'] = len(removed_filters)
            response_data['message'] += f' (removed {len(removed_filters)} contradicting filter(s))'
    ...
    def _mix_pools(self, old_pool, new_filtered_result):
        ...
        logger.debug(f"Pool mixing: {old_track_count} old + {new_track_count} new = {old_track_count + new_track_count} total")
    ...
    def _rebuild_playback_pool(self):
        ...
        for i, filter_record in enumerate(self.filter_queue):
            ...
            if reduction_rate > 0.5:
                ...
                logger.debug(f"Applied {filter_name} with {reduction_rate:.1%} reduction rate using adjusted radius (multiplier: {self.radius_multiplier_factor})")
            else:
    ...
    def clear_adjustment_history(self):
        ...
        if self.genre_pool is not None:
            ...
            pool_size_after = len(self.playback_pool)
            logger.info(f"System reset: removed {filters_count} filters, cleared {shown_count} shown tracks, {pool_size_before} → {pool_size_after} tracks")
        else:
    ...
    def get_shown_tracks_count(self):
        """Get count of tracks that have been shown"""
        return len(self.shown_tracks)
    ...
    def _get_contradicting_adjustment(self, adjustment_id):
        """Get the contradicting adjustment ID for a given adjustment"""
        # Contradicting pairs: (even, odd) for each parameter
        contradicting_pairs = {
            0: 1, 1: 0,  # danceability
            2: 3, 3: 2,  # energy
            4: 5, 5: 4,  # speechiness
            6: 7, 7: 6,  # valence
            8: 9, 9: 8   # tempo
        }
        return contradicting_pairs.get(adjustment_id)
    
    def _remove_contradicting_filters(self, new_adjustment_id):
        """Remove contradicting filters from the queue when adding a new one"""
        contradicting_id = self._get_contradicting_adjustment(new_adjustment_id)
        if contradicting_id is None:
            return []
        
        # Find and remove contradicting filters
        removed_filters = []
        original_length = len(self.filter_queue)
        
        # Keep only filters that don't contradict the new one
        self.filter_queue = [
            filter_record for filter_record in self.filter_queue
            if filter_record['adjustment_id'] != contradicting_id
        ]
        ...
        removed_count = original_length - len(self.filter_queue)
        if removed_count > 0:
            # Create record of removed filters for logging
            removed_filters = [f"adjustment_{contradicting_id}" for _ in range(removed_count)]
            logger.info(f"Removed {removed_count} contradicting filter(s) (adjustment_{contradicting_id}) before adding adjustment_{new_adjustment_id}")
